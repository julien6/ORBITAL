...
def environment(env_type, env_name, env_family, agent_ids, kwargs):
    if env_type == "pz":
        ...
        if SMACliteWrapper is None:
            raise ModuleNotFoundError("smaclite is required for env_type='smaclite'")
        env = SMACliteWrapper(map_name=env_name, agent_ids=agent_ids, **kwargs)
    elif env_type == "lbf":
        if LBFWrapper is None:
            raise ModuleNotFoundError("lbforaging is required for env_type='lbf'")
